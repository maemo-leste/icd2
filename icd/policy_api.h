...
/**
 * @brief  Result of the policy decision
 *
 * @param  status        status of the operation, i.e. whether to accept or
 *                       deny the request
 * @param  new_request   the new connection request or NULL if the request
 *                       has been merged with an existing one
...
/**
 * @brief  New connection request policy function. Note that this function
 *         call is asynchronous; the callback needs to be called in order to
 *         report status.
 *
 ...
 *                            structure
 *
 * @note   Policy modules implementing this function should also check
 *         whether they need to implement #icd_policy_request_cancel_fn.
 */
typedef void
(*icd_policy_request_new_fn) (struct icd_policy_request *new_request,
                              ...
                              gpointer *private);
/**
 * @brief  Clean up internal policy module data structures for a request that
 *         has previously reported #ICD_POLICY_WAITING. After returning from
 *         this function, the request will be freed.
 *
...
/**
 * @brief  Network connection policy function that will be once for each
 *         network that is tried.
 *
...
/**
 * @brief  Network connection restart policy function; decides how many times
...
/**
 * @brief  Informational policy called when a network has been successfully
 *         connected
 *
 * @param  network               the network to connect
 * @param  existing_connections  existing network connections
...
/**
 * @brief  Network disconnection policy function called when ICd attempts to
...
/**
 * @brief  Informational policy called when a network has been disconnected
 *
...
/**
 * @brief  Informationa policy called when a scan is stopped for a network
 *         type
 *
...
/**
 * @brief  Informationa policy called when a scan is started for a network
 *         type
 *
 * @param  network_type  network type
 * @param  private       private data for the module
...
/**
 * @brief  Policy module destruction function. Will be called before
...
/**
 * @brief  Policy module network priority function.
 *
...
/**
 * @brief  Add a network connection to try in response to the policy
 *         decision. Any policy module using this function must be compatible
 *         with the corresponding network module. In order to avoid excessive
 *         string allocation, copying and deletion, ICd - not the module -
 *         will free all strings with g_free().
 *
...
/**
 * @brief  Merge two request together and free the request_to_merge
 *         structure.
 *
 * @param  request_to_merge  the request that will be joined with the
 *                           existing one. DO NOT USE after calling this
 *                           function as it is freed and can point to
 *                           anything afterwards.
...
/**
 * @brief  Make a request for a new network connection. The new request will
...
/**
 * @brief  Callback function for network scan.
 *
 * @param  status            network scan status with ICD_POLICY_SCAN_*
 *                           values
 ...
 * @param  signal            signal leve, see #network_api.h
...
/**
 * @brief  Request network scan. Only usable in #icd_policy_request_new_fn,
 *         as it's the only asynchronous policy function that a policy module
 *         can implement. Note also that scanning will continue until
 *         #icd_policy_scan_stop_fn is called.
 *
...
/**
 * @brief  Close a connected network in icd_policy_nw_(dis)?connect*
 *         functions
 *
 * @param  network  the network to disconnect
 */
...
/**
 * @brief  Policy module initialization function. ICd will look for this type
 *         of function called 'icd_policy_init' for each policy module.
 *
